...
def test_types_are_children_of_pokemon_class():
    ...
    test_pokemon_type_fire = Fire(
        "Charmander", 44, 17, "Flamethrower", "fire", "grass", "water"
    )
    test_pokemon_type_water = Water(
        "Vaporeon", 70, 19, "Hydro pump", "water", "fire", "grass"
    )
    test_pokemon_type_grass = Grass(
        "Leafeon", 65, 17, "Giga drain", "grass", "water", "fire"
    )
    test_pokemon_type_normal = Normal(
        "Eevee", 55, 18, "Headbutt", "normal", None, "Fighting"
    )
...
def test_should_return_the_multiplier_depending_on_the_weak_against():
    ...
    assert (
        test_pokemon_type_fire.get_multiplier(test_pokemon_type_water) == 0.5
    )
    assert (
        test_pokemon_type_grass.get_multiplier(test_pokemon_type_water) == 1.5
    )
    assert (
        test_pokemon_type_normal.get_multiplier(test_pokemon_type_water) == 1
    )
...
def test_is_empty_method_when_pokeball_is_empty():
    ...
    ball_1 = Pokeball()
    assert ball_1.is_empty() is True
    ball_1.catch(test_pokemon_type_fire)
    assert ball_1.is_empty() is False
...
def test_take_turn_correctly_substracts_right_hit_points():
    pok_1 = Water("Vaporeon", 70, 19, "Hydro pump", "water", "fire", "grass")
    pok_2 = Grass("Leafeon", 65, 17, "Giga drain", "grass", "water", "fire")
    battle = Battle(pok_1, pok_2)
    assert (
        battle.take_turn()
        == "Round 1: Leafeon has taken 19.5 damage points, "
        "remaining hit points 45.5"
    )
    assert (
        battle.take_turn()
        == "Round 2: Vaporeon has taken 18.5 damage points,"
        " remaining hit points 51.5"
    )
    assert (
        battle.take_turn()
        == "Round 3: Leafeon has taken 19.5 damage points, "
        "remaining hit points 26.0"
    )
    assert (
        battle.take_turn()
        == "Round 4: Vaporeon has taken 18.5 damage points, "
        "remaining hit points 33.0"
    )
    assert (
        battle.take_turn()
        == "Round 5: Leafeon has taken 19.5 damage points, "
        "remaining hit points 6.5"
    )
    assert (
        battle.take_turn()
        == "Round 6: Vaporeon has taken 18.5 damage points, "
        "remaining hit points 14.5"
    )
...
def test_take_turns_flags_when_the_pokemon_has_fainted():
    ...
    assert (
        battle.take_turn()
        == "Round 7: Leafeon has taken 19.5 damage points, "
        "Leafeon has fainted."
    )
